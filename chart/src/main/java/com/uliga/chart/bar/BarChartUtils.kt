...
internal object BarChartUtils {
  ...
  fun BarChartData.forEachWithVerticalArea(
    drawScope: DrawScope,
    barDrawableArea: Rect,
    progress: Float,
    labelDrawer: LabelDrawer,
    ...
    val totalBars = bars.size
    val heightOfBarArea = barDrawableArea.height / totalBars
    ...
    bars.forEachIndexed { index, bar ->
      ...
      val barArea = Rect(
        left = barDrawableArea.left,
        top = left + heightOfBarArea - offsetOfBar,
        right = barDrawableArea.left + (bar.value / maxBarValue) * barWidth,
        bottom = left + offsetOfBar
      )
